...
int SJFElect(void) {
	printf("SJF Election !\n");
	int p = -1;  //signifie aucun processus choisi
 	long shortestTime = LONGTIME + 1;
	for (int i = 0; i < MAXPROC; i++) {
	    if (Tproc[i].flag == RUN && Tproc[i].duration < shortestTime) {
	      shortestTime = Tproc[i].duration;
	      p = i;
	    }
	  }
  return p;	
...
// Approximation SJF
int ApproxSJF(void) {
	printf("Dynamic Priority Election !\n");
	int p = -1;  
	long shortestTime = LONGTIME + 1;
	for (int i = 0; i < MAXPROC; i++) {
	    if (Tproc[i].flag == RUN && Tproc[i].ncpu < shortestTime){
			shortestTime = Tproc[i].ncpu;
	      	p = i;
    	}
  	}
  	AjustPrio();
